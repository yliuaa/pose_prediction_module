...
def main(opt):
    ...
    if opt.is_load or opt.is_eval:
        ...
        ckpt = torch.load(model_path_len, map_location=torch.device(_DEVICE))
...
def eval(opt):
    ...
    for act in acts:
        ...
        data_loader[act] = DataLoader(dataset, batch_size=opt.test_batch_size, shuffle=True, num_workers=0,
                             pin_memory=True)
    is_create = True
    avg_ret_log = []
    ...
    for act in acts:
        ...
        single_action_time = 1000*(time.time() - single_action_time)
        print(act + " time taken: " + str(single_action_time) + "ms")
    ...
    os.rename('./pgbig_eval_{}_{}.csv'.format(opt.test_batch_size*15, opt.cuda_idx), './pgbig_eval_{}_{}_time_{}.csv'.format(opt.test_batch_size*15, opt.cuda_idx, int(1000*_INFERENCE_TIME)))
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        n += batch_size
        bt = time.time()
        p3d_h36 = p3d_h36.float().to(_DEVICE)
        ...
        if i % 1000 == 0:
            global _INFERENCE_TIME 
            _INFERENCE_TIME += (time.time() - bt)
