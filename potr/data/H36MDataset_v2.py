...
class H36MDataset(torch.utils.data.Dataset):
  ...
  def _sample_batch_eval(self, action):
    ...
    # Getting zeros with target shape
  ...
  def load_action_defs(self):
    ...
    json_config = os.path.join(thispath + '/../' + self._params['data_path'], 'action_defs.json')
  ...
  def load_data(self):
    ...
    file_prefix = "{}/S{}/{}_{}.npy"
    dataset_path = os.path.join(thispath + '/../' +self._params['data_path'], 'dataset')
